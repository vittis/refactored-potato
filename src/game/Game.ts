...
export class Game {
    ...
    hasGameEnded() {
        const allP1Units = this.boardManager.getAllUnits().filter((unit) => unit.owner === "P1");

        const allP2Units = this.boardManager.getAllUnits().filter((unit) => unit.owner === "P2");

        return allP1Units.every((unit) => unit.stats.hp <= 0) || allP2Units.every((unit) => unit.stats.hp <= 0);
    }
}
