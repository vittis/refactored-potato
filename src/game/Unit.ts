...
export class Unit extends Entity {
    ...
    serialize() {
        return {
            ...
            actionTarget: this.currentAction
                ? { col: this.currentAction.getTarget()?.col, row: this.currentAction.getTarget()?.row }
                : undefined,
            equipment: {
                mainHandWeapon: {
    ...
    constructor(owner: "P1" | "P2", col: number, row: number, { backgrounds, equipment }: UnitData) {
        super(col, row);
        this.owner = owner;
        ...
        const totalWeight =
            (equipment?.chest?.weight || 0) + (equipment?.head?.weight || 0) + equipment?.mainHandWeapon?.weight;
    ...
    step(bm: BoardManager) {
        ...
        if (!this.currentAction) {
            const closestEnemyUnitTile = this.getClosestTileWithEnemy(bm, this);
            if (closestEnemyUnitTile) {
                const closestEnemyUnit = closestEnemyUnitTile.entity as Unit;
                this.currentAction = new AttackAction(closestEnemyUnit);
            }
        }
    ...
    isActionValid(bm: BoardManager) {
        ...
        if (this.currentAction?.type === ACTION_TYPE.ATTACK) {
            ...
            if (actionTarget && !bm.tileAt(actionTarget.col, actionTarget.row).walkable) {
                return false;
            }
        }
        return true;
    ...
    public toString = (): string => {
        return this.backgrounds?.[1] ? this.backgrounds?.[1].name[0] : this.backgrounds[0].name[0];
    };
}
