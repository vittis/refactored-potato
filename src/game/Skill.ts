...
export class Skill {
    ...
    moveTowardsClosestEnemy(bm: BoardManager, unit: Unit, closestEnemyUnit: Unit) {
        /* const actionTarget = unit.currentAction?.getTarget();
    if (
      actionTarget &&
      !bm.tileAt(actionTarget.col, actionTarget.row).walkable
    ) { */
        this.action = new MoveAction(bm.getEntityTile(unit), bm.getEntityTile(closestEnemyUnit), bm);
    ...
    moveAwayFromEnemy(brain: Brain, bm: BoardManager, unit: Unit, closestEnemyUnit: Unit, moveAwayRange: number) {
        const safeTile = brain.getTileAwayFromEnemies(bm, unit, moveAwayRange);
        if (bm.isInRange(unit.col, unit.row, closestEnemyUnit.col, closestEnemyUnit.row, moveAwayRange) && safeTile) {
